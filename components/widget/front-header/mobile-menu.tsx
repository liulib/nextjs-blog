...
const MobileMenu: FC<MobileMenuProps> = ({ open }) => {
    return (
        <ul className={cn('md:hidden', { block: open }, { hidden: !open })}>
            {menu.map(item => {
                return (
                    <li className='group flex items-center gap-x-6 border-b border-dashed border-black/30 bg-gray-50 p-3 text-base font-semibold leading-7 text-gray-600 transition-colors hover:bg-gray-200 dark:border-slate-400/40 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700'>
